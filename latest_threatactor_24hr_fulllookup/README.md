...
- **Recent Threat Actor Data**: Fetches threat actors modified in the last 24 hours
...
```bash
python gti_interactive_threat_actor_explorer.py
```
...
You can modify the following constants in the script to adjust behavior:

```python
REQUEST_TIMEOUT = 30        # API request timeout in seconds
MAX_RETRIES = 3             # Maximum retry attempts for API requests
RETRY_DELAY = 5             # Initial delay between retries in seconds
IOC_LIMIT_PER_TYPE = 10     # Maximum IOCs to display per type
RELATIONSHIP_LIMIT = 15     # Maximum relationships to display per type
ACTOR_FETCH_LIMIT = 5       # Number of actors to fetch initially
OUTPUT_DIR = "output"       # Directory for JSON output files
RECENT_THRESHOLD_SECONDS = 86400  # Threshold for highlighting (24 hours)
```
